...
func tiktokStats(rs *response) (hasImages bool, hasPlay bool, imageCount int) {
	if rs == nil {
		return false, false, 0
	}
	imageCount = len(rs.Data.Images)
	return imageCount > 0, rs.Data.Play != "", imageCount
}
...
func handlerInline(ctx context.Context, b *bot.Bot, update *models.Update) {
	...
	hasImages, hasPlay, imageCount := tiktokStats(rs)
	if err != nil || (!hasPlay && !hasImages) {
		status := "success"
		if err != nil {
			status = "error"
		}
